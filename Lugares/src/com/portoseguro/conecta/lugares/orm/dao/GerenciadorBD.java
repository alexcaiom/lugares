...
public class GerenciadorBD extends SQLiteOpenHelper {
	...
	public GerenciadorBD(Contexto contexto) {
	...
	public void onCreate(SQLiteDatabase bd) {
		try {
			...
			bd.execSQL(GeradorSQLBean.getInstancia(Lugar.class).getCreateTable());
	...
	public void onUpgrade(SQLiteDatabase bd, int versaoAnterior, int versaoNova) {
		try {
		bd.execSQL(GeradorSQLBean.getInstancia(Lugar.class).getDropTable());
		onCreate(bd);
		} catch (SQLException e) {
			e.printStackTrace();
		}
